...
$db = new Database();
$name = $address = $email = $user = $password = $role = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = $db->select("SELECT * FROM tbl_user WHERE user = '" . addslashes($user) . "'");
    if ($check && $check->num_rows > 0) {
        $error = 'Tên tài khoản đã tồn tại!';
    } else {
        $sql = "INSERT INTO tbl_user (name, address, email, user, password, role) VALUES ('$name', '$address', '$email', '$user', '$password', $role)";
        $db->insert($sql);
